...
PRECEDENT_LIBRARY = [
    ...
    {"rule": "phosphor", "type": "부정_참고사례_검증필요",
     "description": "phosphor 규칙 중 P=S(포스포로티오에이트) 서브클래스는 valid set 33건 중 "
                     "8건이며, 구조 확인 결과 O,O-디에틸 포스포로티오에이트 골격의 유기인계 "
                     "살충제(파라티온/포레이트/다이설포톤류)로 판단됨. 이들은 애초에 치료용 "
                     "약물 후보가 아니라 환경독성 물질(농약)이므로, '독성을 낮춰 약물로 개선'하는 "
                     "본 파이프라인의 목적 범위 밖. 새 치환 candidate를 만드는 것 자체가 "
                     "부적절하다고 판단해 처리 대상에서 제외, stuck이 정상 결과로 남음."},
    {"rule": "iodine", "type": "위험=메커니즘_참고_검증필요",
     "description": "iodine 규칙은 REPLACEMENT_LIBRARY에 candidate가 없어 자동 치환 "
                     "불가(no_known_fix)로 남음. 다만 ChEMBL 서브구조 검색(디요오도페놀 "
                     "골격) 결과 덱스트로티록신(DEXTROTHYROXINE, 갑상선호르몬 유사체)과 "
                     "아미오다론(AMIODARONE, 항부정맥제) 등 승인약물 확인됨 — 두 약물 "
                     "모두 요오드화 방향족 구조 자체가 약효의 핵심 메커니즘. 향후 자동 "
                     "치환 candidate를 설계할 경우, 요오드 전체 제거보다 개수를 줄이는 "
                     "보수적 접근이나 사람 검토 우선이 필요함을 시사."},
    {"rule": "het_thio_666_A(13)", "type": "위험=메커니즘_참고_검증필요",
     "description": "het_thio_666_A(13) 규칙(페노티아진 골격, S와 N이 벤젠고리 2개를 "
                     "잇는 삼환구조)은 REPLACEMENT_LIBRARY에 candidate가 없어 자동 치환 "
                     "불가(no_known_fix)로 남음. ChEMBL 서브구조 검색 결과 "
                     "트리플루오페라진, 티오리다진, 클로르프로마진, 프로마진, "
                     "퍼페나진, 트리플루프로마진, 프로메타진, 플루페나진, "
                     "프로클로르페라진 등 9개 승인 항정신병약/항히스타민제가 이 골격을 "
                     "그대로 사용 — 매우 강력한 승인약물 근거. 향후 candidate를 설계할 "
                     "경우 삼환 골격 자체를 보존하면서 반응성만 낮추는 정교한 접근이 "
                     "필요하며, 단순 고리 개환/황 치환은 약효 상실 위험이 매우 높음."},
    {"rule": "phosphor", "type": "치환=승인약물_사례있음_검증됨",
     "description": "phosphor 규칙 중 음이온 인산 디에스터(R-O-P(=O)([O-])-O-R', "
                     "생리적 pH에서 이온화된 형태)는 ChEMBL 서브구조 검색(100건 "
                     "표본) 결과 밀테포신(MILTEFOSINE, phase4 승인, 리슈만편모충증 "
                     "치료제, 알킬포스포콜린 계열)에서 실제로 확인됨. 원래 "
                     "problem_smarts([OX2] 중성 산소만 허용)가 이 이온화 형태를 "
                     "놓쳐 stuck으로 남던 문제를 발견, [OX1,OX2]로 확장해 매치되게 "
                     "수정. 다만 1회 cleave_bond 치환 후 생성되는 모노에스터 "
                     "중간산물(R-O-[PH](=O)[O-])이 여전히 phosphor로 재진단되어 "
                     "완전해소로 이어지지 않음 - 이 중간산물에 대한 2단계 후속 "
                     "치환 candidate가 별도로 필요함(미해결, 다음 세션 과제)."},
]
